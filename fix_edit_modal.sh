...
INNER_EOF
sed -i -e '/{videoOptions.map((option, index) => (/,/}))}/c\' -e "$(cat replacement.txt | sed 's/$/\\/')" src/components/EditSubtitleModal.tsx
sed -i 's/}))}\\/}))}/g' src/components/EditSubtitleModal.tsx
